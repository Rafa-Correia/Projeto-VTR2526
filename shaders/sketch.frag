...
uniform vec4 lPos;        // LIGHT POSITION (tipicamente em view-space no Nau3D)
uniform sampler2D tex;    // TEXTURE UNIT 0

// Parametros artistico-tecnicos (podes “hardcodar” ou mapear via outro sistema se tiveres)
uniform vec3  uPaperColor = vec3(0.95);
uniform vec3  uInkColor   = vec3(0.08);

uniform float uLineScale  = 90.0;   // densidade das linhas
uniform float uLineWidth  = 0.11;   // espessura relativa (0..0.5)
...
// thresholds editaveis (quando cada camada comeca a entrar)
...
// largura de transicao (o quao suave e a entrada)
...
float hatchMask(vec2 p, float angle, float scale, float width, float jitter)
{
    ...
    float ink = 1.0 - smoothstep(0.0, width, d);
...
void main()
{
    ...
    float grain = (noise2(p * 7.0) - 0.5) * uPaperGrain;

    // Cross-hatching (4 direcoes)
    float h1 = hatchMask(p, radians(20.0),  uLineScale * 1.00, uLineWidth, uJitter);
    float h2 = hatchMask(p, radians(65.0),  uLineScale * 0.95, uLineWidth, uJitter);
    float h3 = hatchMask(p, radians(110.0), uLineScale * 0.90, uLineWidth, uJitter);
    float h4 = hatchMask(p, radians(155.0), uLineScale * 0.85, uLineWidth, uJitter);
